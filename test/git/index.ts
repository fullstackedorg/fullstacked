...
function resetRepositories() {
    ...
    child_process.execSync("docker compose exec git-server /bin/bash /home/setup.sh", {
...
suite("git - e2e", () => {
    ...
    test("log", async () => {
        ...
        const logsGit = child_process.execSync("git log -n 10", { cwd: testDirectory }).toString();
        const hash = logsGit.match(/commit .*/g)?.[0]?.split(" ")?.pop()?.trim();
        const author = {
            name: logsGit.match(/Author: .*</g)?.[0]?.split(" ")?.slice(1, -1)?.join(" ")?.trim(),
            email: logsGit.match(/<.*>/g)?.[0]?.slice(1, -1)
        }
        const date = logsGit.match(/Date:.*/g)?.[0]?.split("   ")?.pop()?.trim();
    ...
    test("status / add", async () => {
        ...
        const gitStatus = child_process.execSync("git status", { cwd: testDirectory }).toString();

        const branch = gitStatus.match(/On branch.*/g)?.[0]?.split(" ")?.pop()?.trim()
        assert.deepEqual(branch, status.head.branch);
        
        const staged = gitStatus.match(/Changes to be committed:(\s|.)*?\n\n/g)?.[0]?.match(/new file:.*/g)?.[0]?.split(" ")?.pop()?.trim();
        assert.ok(staged);
        assert.deepEqual(staged, status.staged.added.at(0));

        const unstaged = gitStatus.match(/Changes not staged for commit:(\s|.)*?\n\n/g)?.[0]?.match(/modified:.*/g)?.[0]?.split(" ")?.pop()?.trim();
        assert.ok(unstaged);
